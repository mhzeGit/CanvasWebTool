...
export function drawOneNode(idx) {
  const ctx = state.ctx;
  const n = state.nodes[idx];
  ...
  if (state.selected.has(idx)) {
    ...
    const sx = (n.x * state.scale + state.offsetX) * dpr;
    const sy = (n.y * state.scale + state.offsetY) * dpr;
    const sw = n.w * state.scale * dpr;
    const sh = n.h * state.scale * dpr;
    const sr = nodeRadius * state.scale * dpr;
    ctx.strokeStyle = '#f0c800';
    ctx.lineWidth = 1.5;
    drawRoundedRect(ctx, sx, sy, sw, sh, sr);
    ctx.stroke();
    ctx.restore();
  }
...
export function drawNodes(indices) {
  ...
  for (const idx of drawOrder) drawOneNode(idx);
}
